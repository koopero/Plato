...
#
def load_csv_and_update_pcb(csv_file_path, offset = (0,0)):
    ...
    with open(csv_file_path, newline='') as csvfile:
        ...
        for row in reader:
            ...
            if part_name == 'Name':  # Skip the header row
                continue
            x = int((float(x)+ox) * 1000000)  # Convert mm to nm (nanometers)
            y = int((-float(y)+oy) * 1000000)  # Convert mm to nm
...
csv_file_path = '/Users/koop/Code/PlatoGlow/Plato_A2/output/Plato_A2_LEDs.csv'  # Adjust the path as needed
load_csv_and_update_pcb(csv_file_path, offset=(110,60))
